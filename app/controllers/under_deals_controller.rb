class UnderDealsController < ApplicationController
  def index
    ...
    @finish_deals = UnderDeal.includes(:ad,:deal_messages,:owner).references(:ad,:deal_messages,:owner).where(work_status: 'finished')
  end

  def show
  	@under_deal = UnderDeal.includes(:ad,:owner).includes(:ad => :ad_client).find(params[:id])
  	@ad = @under_deal.ad
