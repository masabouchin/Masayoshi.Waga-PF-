class OwnersController < ApplicationController
  ...
  def dashboard
    ...
    @under_deals = UnderDeal.includes(:ad,:deal_messages).references(:ad,:deal_messages).where(owner_id: @owner.id)
  ...
  #オーナー様側の編集ページ表示
  def edit
    @owner = Owner.includes(:transfer_informations).find(params[:id])
  end
